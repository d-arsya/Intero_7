...
export function Header({ links }: HeaderProps) {
  // State untuk menyimpan status login, dropdown, dan nama user
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [isShowDropdown, setIsShowDropdown] = useState(false);
  const [userName, setUserName] = useState("User"); // Default nama user
  ...
  // Fungsi untuk mendapatkan nama user dari cookie (jika ada)
  ...
  // Cek token dan nama user saat komponen dimuat
  useEffect(() => {
  ...
  return (
    <header className="flex justify-between py-6 items-center px-6 md:px-32 sticky top-0 z-50 bg-white shadow-md">
      <Link href="/donasi" className="flex items-center gap-x-2">
        <Image
        ...
        <span className="text-navy font-bold text-xl md:text-2xl">
          BBJ <span className="text-[#F5B041]">Donate</span>
        </span>
      </Link>
      {isLoggedIn && (
        <nav className="flex ms-auto space-x-12 items-center">
          ...
          <Link
            className="bg-[#F5B041] text-sm rounded-full h-max py-2 text-white font-semibold px-4 md:px-6 hidden md:block"
            href="/donasi"
          >
            Donasi Sekarang
          </Link>
          {/* Icon user (hanya muncul jika sudah login) */}
          ...
          <div className="cursor-pointer relative">
            ...
            {/* Dropdown untuk nama user dan logout */}
            {isShowDropdown && (
              <div className="w-[150px] bg-white drop-shadow-md right-0 top-12 rounded-lg flex flex-col absolute p-4 z-50">
                <p className="text-sm font-semibold mb-2">{userName}</p>
                <button
                  className="w-full h-10 bg-red-500 text-white rounded-md hover:bg-red-600 transition"
